    ...
    fn write(&mut self, buf: &[u8]) -> std::io::Result<usize> {
        let str = buf
            ...
            .map(|c| {
                ...
                return match ALPABETH
                    .iter()
                    .position(|&letter| letter == lower_case_char)
                {
                    Some(i) => {
                        if c.is_uppercase() {
                            switch(i).to_ascii_uppercase()
                        } else {
                            switch(i)
                        }
                    }
...
fn main() {
    ...
    buff.write(b"lbh penpxrq z1 fhcre qvssvphyg pbqvat punyyratr... pbqr vf ddommNst")
        .unwrap();
    println!(
        "result: {:?}",
...
mod tests {
    ...
    fn test_rod13() {
        ...
        assert_eq!(
            content.iter().map(|x| *x as char).collect::<String>(),
            "nopqrstxyzavcdefghijklm.123456789 "
        )
    }
}
